...
| 🔎 가전 신제품 조사 | 대륙별 탑 5 가전사 홈페이지에서 품목별 신제품을 찾아 **가격·POD·스펙**을 정리하고 가격대별로 비교 | `:9119` | `appliance_watch/` |
...
appliance_watch/ 가전 신제품 조사 (catalog.py 대륙·탑5·품목·가격대 표, discover.py 출처 찾기,
...
대륙별 탑 5 가전사의 **새로 나온 제품**을 공식 홈페이지에서 찾아 가격·POD(차별점)·스펙을
정리하고, **가격대별로** 나란히 비교합니다. 화면은 `http://localhost:3000/appliances` 입니다.

```
북아메리카 + 조리기기 + [GE, Whirlpool, Samsung, LG, Frigidaire] → «신제품 찾기»
...
- 대륙별 탑 5 와 홈페이지 주소는 기본값입니다. `set_region_makers` 로 회사 기준에 맞게 바꾸세요.
