...
| 🔎 가전 신제품 조사 | 영향력 기준 글로벌 탑 20 가전사 홈페이지에서 품목별 신제품을 찾아 **가격·POD·에너지효율·AI기능·스펙**을 정리하고 가격대별로 비교 | `:9119` | `appliance_watch/` |
...
북아메리카 + 조리기기 + [글로벌 톱티어 10개: Samsung, LG, Whirlpool, GE, Bosch...] → «신제품 찾기»
   ...
   → 제품 페이지에서 가격·모델·스펙·특징·에너지 효율을 읽고
   → 경쟁사에 없는 특징을 POD 로, AI·음성·앱연동 기능을 따로 뽑고
   → 보급형 / 중급형 / 프리미엄 / 최고급 칸에 나눠 담습니다
```

**POD·AI 기능·에너지 효율은 이렇게 정리합니다** (`pod.py` 의 `enrich_product`)

- 사내 LLM(Gauss 등, `LLM_BASE_URL`)이 붙어 있으면: 제품 하나당 호출 한 번으로
  POD·AI 기능·에너지 효율을 같이 정형화해서 받습니다. 페이지에 근거가 있는
  것만 쓰게 하고, 셋 다 비면 응답을 못 믿는다고 보고 규칙 기반으로 넘어갑니다.
- 없거나 실패하면: POD 는 경쟁 제품에 잘 안 나오는 낱말이 많은 특징 문장을,
  AI 기능은 "AI·음성제어·앱연동·사물인터넷"처럼 구체적인 표현이 있는 문장만
  골라 냅니다("스마트"/"자동" 처럼 흔한 낱말 하나만으로는 오탐이 많아서 안
  잡습니다). 에너지 효율은 페이지의 스펙 표·배지·설명에서 "Energy Star",
  "220 kWh/yr", "Energy Class A+++" 같은 표현을 그대로 가져옵니다
  (`extract.py` 의 `_energy_rating`).
- 페이지에서 이미 규칙으로 찾은 값(특히 에너지 효율)은 LLM 이 덮어쓰지
  않습니다 — 스크래핑이 사실을 담당하고 LLM 은 정리·분류만 맡습니다.
